...
srun --cpu-bind=none --container-name RaySpace \
     ...
     bash -c "source /sc/home/anton.hackl/conda3/etc/profile.d/conda.sh && conda activate spatial_benchmark && export PYTHONPATH=\$PYTHONPATH:. && export CGAL_THREADS=\${SLURM_CPUS_PER_TASK:-20} && export OMP_NUM_THREADS=\$CGAL_THREADS && export OMP_DISPLAY_ENV=VERBOSE && echo \"[OpenMP] SLURM_CPUS_PER_TASK=\${SLURM_CPUS_PER_TASK:-unset} OMP_NUM_THREADS=\$OMP_NUM_THREADS\" && python benchmarks/mesh_intersection/run_mesh_complexity_benchmark.py --runs 3 --timeout 1200 --threads \$CGAL_THREADS"
