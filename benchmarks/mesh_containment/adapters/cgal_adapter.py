...
class CGALContainmentAdapter(ContainmentBenchmarkAdapter):
    def __init__(
        self,
        cgal_dir: str,
    ...
    def run_containment(
        ...
        print(
            f"[{self.name}] Running containment benchmark on {p1.name} and {p2.name} "
        ...
        for run_idx in range(num_runs):
            try:
                ...
                if run_idx == 0:
                    ...
                    if count_match:
                        num_containments = int(count_match.group(1))
