...
CGAL_EXEC = CGAL_DIR / "build" / "cgal_containment"
...
def _run_rayspace_containment_pairs(
    pre_mesh_a: Path,
    pre_mesh_b: Path,
    output_csv: Path,
    ...
    executable = RAYSPACE_DIR / "query" / "build" / "bin" / "raytracer_containment"
    if not executable.exists():
        return {"error": f"Executable not found: {executable}"}

    default_csv = run_dir / "mesh_containment_results.csv"
    ...
    cmd = [
        ...
        "1",
        "--warmup-runs",
        str(max(0, warmup_runs)),
    ]
    if use_anyhit_point_in_mesh:
    ...
    try:
        ...
        return {"error": f"RaySpace containment timed out ({TIMEOUT_SECONDS}s)"}
    except subprocess.CalledProcessError as exc:
        return {"error": f"RaySpace containment failed with exit code {exc.returncode}: {exc.stderr}"}

    output = cp.stdout + cp.stderr

    if not default_csv.exists():
        return {"error": f"RaySpace containment did not produce expected CSV at {default_csv}"}

    if output_csv.exists():
        output_csv.unlink()
    default_csv.rename(output_csv)

    count_match = re.search(r"Containment pairs \(B in A\):\s*(\d+)", output)
...
def _run_cgal_containment_pairs(
    pre_mesh_a: Path,
    pre_mesh_b: Path,
    output_csv: Path,
    ...
    try:
        ...
        return {"error": f"CGAL containment failed: {exc}"}

    output = stdout_text + stderr_text
    if not output_csv.exists():
        return {
            "error": (
                "CGAL containment did not produce a pair CSV. Ensure CGAL is rebuilt with "
                "containment_query.cpp support for --output-csv."
            ),
...
def _is_intersection(a: AABB, b: AABB, eps: float = 1e-9) -> bool:
    overlap_x = _axis_overlap(a.min_x, a.max_x, b.min_x, b.max_x)
    overlap_y = _axis_overlap(a.min_y, a.max_y, b.min_y, b.max_y)
    overlap_z = _axis_overlap(a.min_z, a.max_z, b.min_z, b.max_z)
    return overlap_x > eps and overlap_y > eps and overlap_z > eps


def _is_strict_containment(a: AABB, b: AABB, eps: float = 1e-9) -> bool:
    return (
        (a.min_x + eps) < b.min_x
        and (a.min_y + eps) < b.min_y
        and (a.min_z + eps) < b.min_z
        and (a.max_x - eps) > b.max_x
        and (a.max_y - eps) > b.max_y
        and (a.max_z - eps) > b.max_z
    )


def _categorize_pair(a: AABB, b: AABB, eps: float = 1e-9) -> str:
    if _is_strict_containment(a, b, eps=eps):
        return "strict_containment"
    if _is_intersection(a, b, eps=eps):
        return "overlap_not_containment"
    return "disjoint_or_touch"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run RaySpace and CGAL containment on cubes_20k (or consume existing pair CSVs), "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional existing RaySpace containment pairs CSV to analyze (skips running RaySpace query)",
    )
    parser.add_argument(
        "--cgal-pairs-csv",
        type=str,
        default=None,
        help="Optional existing CGAL containment pairs CSV to analyze (skips running CGAL query)",
    )
    parser.add_argument(
        "--use-anyhit-point-in-mesh",
        action="store_true",
        help="Run RaySpace containment with AnyHit point-in-mesh mode",
    )
    ...
    run_layout = create_benchmark_run_layout(SCRIPT_DIR, "containment_disagreement")
    run_name = run_layout["run_name"]
    run_dir = Path(run_layout["run_dir"])

    rs_pairs_csv = run_dir / "rayspace_containment_pairs.csv"
    rs_timing_json = run_dir / "rayspace_containment_timing.json"
    cgal_pairs_csv = run_dir / "cgal_containment_pairs.csv"
    cgal_log = run_dir / "cgal_containment.log"
    details_csv = run_dir / "sampled_pair_decisions.csv"
    report_json = run_dir / "summary.json"
    results_json = Path(run_layout["results_json"])
    latest_json = RUNS_DIR / "containment_disagreement_latest.json"
    ...
    if use_existing_pair_csvs:
        ...
        rs_result = _run_rayspace_containment_pairs(
            pre_a,
            pre_b,
            rs_pairs_csv,
            rs_timing_json,
            run_dir,
            args.warmup_runs,
            args.use_anyhit_point_in_mesh,
        )
        if "error" in rs_result:
            raise RuntimeError(f"RaySpace containment failed: {rs_result['error']}")

        print("Running CGAL containment with pair export...")
        cgal_result = _run_cgal_containment_pairs(pre_a, pre_b, cgal_pairs_csv, args.threads, cgal_log)
    ...
    for (a_id, b_id), bucket in sampled:
        ...
        if a_box is None or b_box is None:
            winner = "inconclusive"
            truth = None
            category = "missing_aabb"
            inconclusive += 1
            category_counts[category] += 1
        else:
            truth = _is_strict_containment(a_box, b_box)
            category = _categorize_pair(a_box, b_box)
            category_counts[category] += 1
        ...
        decisions.append(
            {
                ...
                "b_object_id": b_id,
                "bucket": bucket,
                "aabb_relationship": category,
                "truth_containment": truth,
                "winner": winner,
            }
        )
    ...
    summary = {
        "metadata": {
            ...
            "rayspace_anyhit_point_in_mesh": args.use_anyhit_point_in_mesh,
        },
        "pair_counts": {
            "rayspace_pairs": len(rs_pairs),
            "rayspace_pairs_reported": rs_result.get("num_pairs_reported"),
            "cgal_pairs": len(cgal_pairs),
            "cgal_pairs_reported": cgal_result.get("num_pairs_reported"),
            "agreed_pairs": agreed,
        ...
        "sample": {
            ...
            "aabb_relationship_counts": category_counts,
        },
        "adjudication": {
            ...
            "evaluated": evaluated,
            "rayspace_correct_rate": (rayspace_correct / evaluated) if evaluated > 0 else None,
            "cgal_correct_rate": (cgal_correct / evaluated) if evaluated > 0 else None,
        },
        "artifacts": {
            "rayspace_pairs_csv": rs_result.get("pairs_csv"),
            "rayspace_timing_json": str(rs_timing_json) if rs_timing_json.exists() else None,
            "cgal_pairs_csv": cgal_result.get("pairs_csv"),
            "cgal_log": str(cgal_log) if cgal_log.exists() else None,
            "sampled_decisions_csv": str(details_csv),
        },
        "notes": [
            "Containment adjudication uses strict AABB containment (B strictly inside A) for cube datasets.",
            "This report estimates which engine is more correct on a sampled subset of disagreements.",
        ],
    }
    ...
    print("\n=== Containment Disagreement Analysis ===")
